...
export default function FAQPage(){return <PublicShell><main className="min-w-0 mx-auto max-w-3xl px-5 py-16 lg:px-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-emerald-500">FAQ</p><h1 className="mt-2 text-4xl font-black tracking-tight">Questions, answered.</h1><div className="mt-8 space-y-3">{faqs.map(([q,a])=><details key={q} className="rounded-2xl border border-border bg-card p-5"><summary className="cursor-pointer font-bold">{q}</summary><p className="mt-3 text-sm leading-6 text-muted-foreground">{a}</p></details>)}</div></main></PublicShell>}
